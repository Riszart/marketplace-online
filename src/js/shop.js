...
async function showProduct(itemShow){
	...
	user.productsWrapper.innerHTML=''
	products.forEach((element)=>{
		if(element.category == itemShow || itemShow === 'all'){
			...
			user.productsWrapper.appendChild(article);
		}
		}
	)
}
...
function notificationProductAdd(product){
  ...
  <path d="M10.97 4.97a.235.235 0 0 0-.02.022L7.477 9.417 5.384 7.323a.75.75 0 0 0-1.06 1.06L6.97 11.03a.75.75 0 0 0 1.079-.02l3.992-4.99a.75.75 0 0 0-1.071-1.05z"/>
	...
	shop.containerMainBuy.appendChild(article)
	document.addEventListener('scroll', scroll)

	function scroll(){
		if(index.mainEsc.getBoundingClientRect().top > 0){
			article.style.top = '55px' 
		}
		else if(index.mainEsc.getBoundingClientRect().top < 0){
